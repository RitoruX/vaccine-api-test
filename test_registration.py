...
class TestRegistrationApi(unittest.TestCase):
    ...
    def sample_body(self, address = "Bangkok", birth_date = "2000-02-02", citizen_id = "1234567666666", name = "SuphanburiPrajantakham", occupation = "student", surname = "Wang", is_risk = False, phone_number = "0951903000"):
    ...
    def setUp(self):
        ...
        requests.delete("https://wcg-apis.herokuapp.com/registration/1234567666666")
        self.INVALID_ID = {'feedback': 'registration failed: invalid citizen ID'}
        self.SUCCESS = "registration success!"
    ...
    def test_post_registration(self):
        ...
        self.assertEqual(self.response.json()['feedback'], self.SUCCESS)
    ...
    def test_post_registration_with_not_boolean_is_rick(self):
        """Test registration with a not boolean is_risk parameter. (They should be fail to registration.)"""
        self.response = requests.post(API_URL, self.sample_body(is_risk = "True"))
        self.assertEqual(self.response.status_code, 201)
        self.assertNotEqual(self.response.json(), self.SUCCESS)
